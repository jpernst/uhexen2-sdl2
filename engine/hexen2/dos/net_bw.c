 ...
 * $Id: net_bw.c 5450 2015-09-28 00:02:13Z sezero $
 *
...
void BW_Listen (qboolean state)
{
	...
	if (state)
	{
		...
		if ((net_acceptsocket = BW_OpenSocket (net_hostport)) == -1)
			Sys_Error ("%s: Unable to open accept socket\n", __thisfunc__);
		return;
	}
...
int BW_OpenSocket (int port)
{
	...
	if (dos_int86(0x21))
	{
		Con_Printf("BW_OpenSocket failed: %d\n", BW_TranslateError(regs.x.ax));
		return -1;
	}
	s = regs.x.ax;
...
int BW_Read (int s, byte *buf, int len, struct qsockaddr *from)
{
	...
	if (dos_int86(0x21))
	{
		Con_Printf("BW UDP read error: %d\n", BW_TranslateError(regs.x.ax));
		return -1;
	}
...
int BW_Broadcast (int s, byte *msg, int len)
{
	...
	if (dos_int86(0x21))
	{
		Con_Printf("BW_Broadcast failed: %d\n", BW_TranslateError(regs.x.ax));
		return -1;
	}
...
const char *BW_AddrToString (struct qsockaddr *addr)
{
	...
	q_snprintf(buffer, sizeof(buffer), "%d.%d.%d.%d:%d",
