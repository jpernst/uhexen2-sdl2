 ...
 * $Id: d_scan.c 5976 2017-09-16 20:03:22Z sezero $
 *
...
static void D_DrawTurbulent8TSpan (void)
{
	...
	do
	{
		...
		if (scanList[r_turb_spancount-1] == 1)
		{
...
void Turbulent8 (surf_t *s)
{
	...
	do
	{
		...
		if (r_transwater.integer && s->flags & SURF_TRANSLUCENT)
		{
			...
			if (ZScanCount)
				memcpy(origscanList,scanList,count);
		}
	...
	// calculate the initial s/z, t/z, 1/z, s, and t and clamp
		...
		// calculate s and t at the far end of the span
			...
			if (r_transwater.integer && s->flags & SURF_TRANSLUCENT)
			{
				if (ZScanCount)
				{
					memcpy(scanList,&origscanList[count],r_turb_spancount);
