/*
 * net_comx.c -- TTY backend for the dosquake serial network driver.
 * from quake1 source with minor adaptations for uhexen2.
 * $Id: net_comx.c 5004 2012-11-05 16:38:26Z sezero $
 *
...
static void TTY_SetComPortConfig (int portNumber, int port, int irq, int baud, qboolean useModem)
{
	...
	Cvar_SetValueQuick (&config_com_port, (float)port);
	Cvar_SetValueQuick (&config_com_irq, (float)irq);
	Cvar_SetValueQuick (&config_com_baud, (float)baud);
	Cvar_SetValueQuick (&config_com_modem, temp);
}
...
static void TTY_SetModemConfig (int portNumber, const char *dialType, const char *clear, const char *init, const char *hangup)
{
	...
	Cvar_SetQuick (&config_modem_dialtype, dialType);
	Cvar_SetQuick (&config_modem_clear, clear);
	Cvar_SetQuick (&config_modem_init, init);
	Cvar_SetQuick (&config_modem_hangup, hangup);
}
...
static void Modem_Hangup2 (void *p);
static void Modem_Hangup3 (void *p);
static void Modem_Hangup4 (void *p);
...
static void Modem_Hangup (void *p)
{
	...
	disable();
	_p->modemRang = false;
	_p->outputQueue.head = _p->outputQueue.tail = 0;
	_p->inputQueue.head = _p->inputQueue.tail = 0;
	outportb(_p->uart + MODEM_CONTROL_REGISTER, inportb(_p->uart + MODEM_CONTROL_REGISTER) & ~MCR_DTR);
	enable();
	_p->poll.procedure = Modem_Hangup2;
	_p->poll.arg = (void *) _p;
...
{
	ComPort	*_p = (ComPort *) p;
	Modem_Command(_p, _p->shutdown);
	_p->poll.procedure = Modem_Hangup4;
	SchedulePollProcedure(&_p->poll, 1.5);
}
