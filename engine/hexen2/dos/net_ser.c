 ...
 * $Id: net_ser.c 4767 2012-06-16 20:48:51Z sezero $
 *
...
static SerialLine	serialLine[NUM_COM_PORTS];
...
void Serial_SearchForHosts (qboolean xmit)
{
	...
	sprintf(hostcache[hostCacheCount].name, "COM%d", n + 1);
...
static qsocket_t *_Serial_CheckNewConnections (SerialLine *p)
{
	...
	p->connecting = false;
	p->sock->lastMessageTime = net_time;
	sprintf(p->sock->address, "COM%d", (int)((p - serialLine) + 1));
